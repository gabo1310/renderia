<script lang="ts">
    ...
    async function handleSendPrompt() {
        ...
        try {
            const response = await fetch('/api/conversations/send-prompt', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ prompt: promptText, conversation_id: activeConversation.id })
            });
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
<PromptModal {showPromptModal} bind:promptText on:send={handleSendPrompt} on:close={closePromptModal} />
